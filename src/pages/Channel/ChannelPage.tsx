...
import {updateLastSeenChannelId} from "../../store/slice/channelSlice.ts";
...
const ChannelPage: React.FC = () => {
  ...
  const channelPersistModule=useSelector((state:RootState)=>state.channel)
    ...
    useEffect(() => {
        ...
        if(channelId == undefined && usersChannels.channelData?.channels_list){
            for(let ind=0; ind<usersChannels.channelData.channels_list.length; ind++){
                if(usersChannels.channelData.channels_list[ind].ch_id == channelPersistModule.lastSelectedChannel) {
                    handleChannelListOnClick(channelPersistModule.lastSelectedChannel)
  ...
  return (
    <div className="flex">
      ...
      <div className={`grow w-full md:max-w-[20vw] h-[94vh] md:flex flex-col border-s-4  py-4 px-3 border-solid ${extendedSideBarActiveState ? "flex" : "hidden"}`}>
